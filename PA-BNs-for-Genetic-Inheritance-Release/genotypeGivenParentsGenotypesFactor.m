...
n_size = size(genotypesToAlleles,1);
genotypeFactor.card = [n_size, n_size, n_size];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
for i=1:n_size
  for j=1:n_size
    allele_par1 = genotypesToAlleles(i,:);
    allele_par2 = genotypesToAlleles(j,:);
    combination = [[allele_par1(1),allele_par2(1)];[allele_par1(1),allele_par2(2)];[allele_par1(2),allele_par2(1)];[allele_par1(2),allele_par2(2)]];
    for k=1:n_size
      allele_child = genotypesToAlleles(k,:);
      if allele_child(1) == allele_child(2)
        p = sum( ismember(combination, [allele_child(1), allele_child(1)], 'rows'))*0.25;
      else
        p = (sum( ismember(combination, [allele_child(1), allele_child(2)], 'rows')) + sum( ismember(combination, [allele_child(2), allele_child(1)], 'rows')))*0.25;
      end
