...
for i=1:3*numPeople
  if i <= numPeople
    ...
    if (parents(1) == 0) && (parents(2) == 0)
      temp = childCopyGivenFreqsFactor(alleleFreqs, i);
    else
      temp = childCopyGivenParentalsFactor(numAlleles, i, parents(1), parents(1)+numPeople);
    end
  elseif (i > numPeople) && (i <= 2*numPeople)
    ...
    if (parents(1) == 0) && (parents(2) == 0)
      ...
      temp = childCopyGivenParentalsFactor(numAlleles, i, parents(2), parents(2)+numPeople);
    end
  else
    temp = phenotypeGivenCopiesFactor(alphaList, numAlleles, i-2*numPeople, i-numPeople, i);
  end
  factorList(i) = temp;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  