...
for i=1:length(geneCopyVarOneList)
  card = [card, max(size(alleleWeights{i}))];
end
phenotypeFactor.card = [2, card, card];
...
for i=1:prod(phenotypeFactor.card)
  sumWeights = 0;
  for k=1:length(geneCopyVarOneList)
    sumWeights = sumWeights + alleleWeights{k}(assignment(i, k + 1));
  end
  for k=1:length(geneCopyVarTwoList)
    sumWeights = sumWeights + alleleWeights{k}(assignment(i, k + length(geneCopyVarOneList) + 1));
  end
  if assignment(i,1) == 1
    phenotypeFactor.val(i) = computeSigmoid(sumWeights);
  else
    phenotypeFactor.val(i) = 1 - computeSigmoid(sumWeights);
  end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%